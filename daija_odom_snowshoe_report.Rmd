...
hare_weights <- juvenile_hares %>% # create subset of data only containing... 
  ...
  mutate(sex = case_when( # add a new column changing "f" to female and "m" to male
    sex == "f" ~ "Female", # call on "f"
    sex == "m" ~ "Male", # call on "m"
    )) %>% # #If anything else is the case (not male or female) use TRUE
...
Between the sites there is not significant variation in male and female weight.
...
There is an actual difference of 90.47 (g) in means. Mean hare weights for female juvenile snowshoe hares (855.39 g $\pm$ 292.25 g, n = 200) were only slightly smaller than those of male juvenile snowshoe hares (945.86 $\pm$ 333.22 cm, n = 163), a difference of just over 10% (values are mean $\pm$ 1 standard deviation; t(325.02) = 2.71, p = 0.007). The effect size was small (Cohen's *d* = `r hare_weight_cohen$estimate`)


#### D. Relationship between juvenile weight and hind foot length

```{r, fig. cap = "**Figure 4. Relationship beteeen hind foot length (mm) and weight change (g) for the juvenile snowshoe hares. Points indicate individual hare measurements (female = red circles; male = green circles; and NA = blue circles). Linear model summary: $\beta$  "}
ggplot(data = juvenile_hares, aes(x = hindft, y = weight)) + 
  geom_point(aes(color = sex)) + # changed shape of plotted points
